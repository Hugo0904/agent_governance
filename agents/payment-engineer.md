...
**套用時機與判定原則：**
- 當任務內容被判定為支付/金流系統、支付網關串接、代收/代付流程、商戶設定、支付回調或其他支付資料流工作時，可參考本規範。
- 判定應以任務檔與上下文的整體內容為準，不應只靠單一關鍵字、固定廠商代號或單一句型觸發。
- 若只是局部提到支付名詞，但主要工作不在支付核心流程，可只局部參考本規範，無須強制套用完整金流流程。

**金流廠商串接規則：**
- 若任務是接入新廠商，需先從任務檔、文件、後台資訊或使用者補充內容確認廠商代號
- 需確認是 ATM 還是 CVS 類型
...
**工作流程：**
1. **優先檢查進度筆記**: 當任務已被判定需參考本規範時，先檢查 `<AI_AGENT_HUB_ROOT>/progress/` 目錄
